...
let guess = 0;
let selectedColor = '';
let bGround = 'rgba(0, 0, 0, 0) linear-gradient(rgb(153, 69, 48), rgb(168, 72, 47)) repeat scroll 0% 0% / auto padding-box border-box';
$('.submit-btn').hide();
let clickCount = 0;
...
// rend accessible les lignes suivantes
for(let i = 0; i < 10; i++) {
...
// click sur les choix couleurs et permet d'accéder aux prochainnes lignes
...
// selection du jeton à jouer et sa couleur
$('.selector-inner').click(function () {
  isSelected = true;
  $('.selector-outer').css('background', '#994530');
  let peg = ($(this).parent())[0];
  selectedColor = $(this).css('background-color');
  $(peg).css('background-color', selectedColor);
});

// change la couleur du jeton selectionné
...
// effectue la selection des couleurs à découvrir 
function makeAnswer() {
  let ray = [];
  for(let i = 0; i < 4; i++) {
...
// attribution des couleurs et leurs valeurs
